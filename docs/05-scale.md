...
Job → slot 为 `hash(jobId) % N`，**日常不改 N**。

- **加执行者**：把若干 `slot_no` 的归属改到新 `executor_id`（持锁或先停旧进程）。ZSET 数据不动。
- **执行者上限**：同时持有独占 slot 时，活执行者数 ≤ N。默认 N=32。
- **改 N**：停机、清空 ZSET、全量重建、重写归属。改 N 会让几乎所有 Job 换桶，必须搬家而不能只改配置数字。
