...
- MySQL + Redis，合在模块 **`taskx-meta`**（不再拆 `taskx-storage-mysql` / `taskx-storage-redis`）。不引入 ZK。
...
- `(jobId, fireTime)` 唯一。
- 执行权：`PENDING → RUNNING` CAS。允许同一 Job **重叠执行**。
...
持 **slot 锁**（TTL + 看门狗）：ZRANGE 到期项 → 可并发处理每条：停用则 ZREM，否则记下 fireTime、**同步 ZADD 下次** → **再异步** 插库 + submit → 本轮 ZADD 结束后解锁。
...
- **N 个 slot**（默认 32，可配置）。`hash(jobId) % N`。加机器只改归属，不改 N。
