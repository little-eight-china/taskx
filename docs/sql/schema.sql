...
-- 可变草稿。revision 用于乐观锁；定义整图和 UI 布局分开存。
...
-- 发布快照。PUBLISHED 后不更新 definition_json。
...
-- 节点的一次逻辑激活；循环/扇出以后可为同一 node_id 产生多个 activation_no。
...
-- 每次真实尝试独立留痕，不能只在 activation 上累加 retry_times。
...
-- source activation + edge 唯一，保证事务重试不会重复推进下游。
...
-- 与运行态同事务写；发布器幂等 ZADD 到 Redis 后标记。
...
-- WAIT/CALLBACK 预留；只存 token 哈希，原始 token 仅返回给调用方。
